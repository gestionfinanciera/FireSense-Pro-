...
const GeneratorScreen: React.FC<GeneratorScreenProps> = ({ language, onBack, onSaveConfig }) => {
  ...
  const [sensitivities, setSensitivities] = useState<SensitivitySet>({
    general: 0, redDot: 0, mira2x: 0, mira4x: 0, sniper: 0, cam360: 0, buttonSize: 0, dpi: null
  });
  ...
  const detectDevice = () => {
    try {
      ...
      let os = "android";
      if (ua.match(/iPhone|iPad|iPod/i)) { os = "ios"; model = "iPhone Pro"; }
      setDeviceInfo({ model, os, screenInfo: "6.7\", 2796x1290" });
    } catch (e) { Analytics.logError("Error detecting device"); }
  };

  const generateSens = () => {
    // Generar valores realistas entre 140 y 200 para el estilo "Insano"
    const config: SensitivitySet = {
      general: Math.floor(Math.random() * (200 - 170 + 1)) + 170,
      redDot: Math.floor(Math.random() * (200 - 165 + 1)) + 165,
      mira2x: Math.floor(Math.random() * (195 - 160 + 1)) + 160,
      mira4x: Math.floor(Math.random() * (190 - 150 + 1)) + 150,
      sniper: Math.floor(Math.random() * (160 - 120 + 1)) + 120,
      cam360: Math.floor(Math.random() * (200 - 180 + 1)) + 180,
      buttonSize: Math.floor(Math.random() * (60 - 45 + 1)) + 45,
      dpi: dpiEnabled ? 600 : null
    };
  ...
  const handleSave = () => {
    ...
    onSaveConfig({ ...sensitivities, id: Date.now().toString(), name: configName, device: deviceInfo.model, date: new Date().toLocaleDateString() });
    setIsModalOpen(false); setConfigName('');
  };
    ...
    <div className="min-h-screen bg-white flex flex-col animate-in fade-in duration-300" dir={isArabic ? 'rtl' : 'ltr'}>
      <header className="header-gradient pt-12 pb-6 px-6 shadow-md flex items-center sticky top-0 z-20">
        ...
        <div className="flex-1 text-center pr-10 rtl:pr-0 rtl:pl-10">
          ...
          <p className="text-white/80 text-[10px] font-bold uppercase mt-1">Scale: 0-200</p>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="space-y-4">
           ...
           <div className="flex items-center justify-between w-full bg-gray-50 p-5 rounded-[2rem] border border-gray-100">
              ...
              <button onClick={() => setDpiEnabled(!dpiEnabled)} className={`w-14 h-7 rounded-full relative transition-all duration-300 ${dpiEnabled ? 'bg-[#FF1E1E]' : 'bg-gray-200'}`}>
      ...
      <footer className="bg-white border-t border-gray-50 flex items-center h-44 px-4 pb-24">
        <button onClick={generateSens} className="flex-1 flex flex-col items-center group active:scale-95 transition-all">
           <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center mb-1 group-active:bg-red-50 transition-colors">
             <svg className="w-6 h-6 text-gray-400 group-active:text-[#FF1E1E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
           </div>
           <span className="text-[10px] font-black text-gray-400 uppercase">{text.genRegenerate}</span>
        </button>
        <button onClick={() => hasGenerated && setIsModalOpen(true)} disabled={!hasGenerated} className="flex-1 flex flex-col items-center group active:scale-95 transition-all">
           <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center mb-1 group-active:bg-red-50 transition-colors">
             <svg className="w-6 h-6 text-gray-400 group-active:text-[#FF1E1E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
           </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6 bg-black/30 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white border-gray-100 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl border">
              <h2 className="text-xl font-black text-gray-900 mb-6 uppercase tracking-tight text-center">{text.saveModalTitle}</h2>
              <input type="text" value={configName} onChange={(e) => setConfigName(e.target.value)} placeholder={text.saveModalPlaceholder} className="w-full bg-gray-50 border-gray-100 text-gray-800 border-2 rounded-2xl py-4 px-6 text-center font-black focus:border-[#FF1E1E] outline-none transition-all mb-8 uppercase" />
              <div className="flex space-x-3 rtl:space-x-reverse">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 bg-gray-100 text-gray-500 rounded-2xl font-black uppercase text-xs">VOLVER</button>
                <button onClick={handleSave} className="flex-1 py-4 bg-[#FF1E1E] rounded-2xl font-black text-white uppercase text-xs shadow-lg">GUARDAR</button>
              </div>
           </div>
        </div>
      )}

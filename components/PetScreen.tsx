  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] flex flex-col animate-in fade-in duration-300" dir={isArabic ? 'rtl' : 'ltr'}>
      <header className="header-gradient pt-12 pb-6 px-6 shadow-md flex items-center sticky top-0 z-20">
        ...
        <h1 className="text-white font-black text-xl tracking-tight uppercase flex-1 text-center pr-10">Mascota</h1>
      </header>
      <main className="p-4 grid grid-cols-3 gap-3">
        {PETS.map((pet) => (
          <button 
            ...
            className="bg-white rounded-2xl border border-gray-100 overflow-hidden flex flex-col shadow-sm transition-all hover:border-[#FF1E1E]/50 active:scale-95"
          >
            <div className="flex-1 p-2 flex items-center justify-center min-h-[100px]">
              <img src={pet.imageUrl} alt={pet.name} className="w-full h-auto object-contain" />
            </div>
            <div className="bg-[#FF1E1E] py-2 text-center">
              <span className="text-white text-[9px] font-black uppercase truncate block px-1">{pet.name}</span>
            </div>
          </button>
        ))}
      </main>
